...
public class ListBaseAdapter<T> extends BaseAdapter {
	...
	public ListBaseAdapter() {
		_loadmoreText = R.string.message_loading;
		_loadFinishText = R.string.message_loading_no_more;
		_noDateText = R.string.message_no_data;
	}
		...
		if (position == getCount() - 1) {// 最后一条
			if (getState() == STATE_LOAD_MORE || getState() == STATE_NO_MORE
				...
				ProgressBar progress = (ProgressBar) mFooterView
						.findViewById(R.id.message_progressbar);
				TextView text = (TextView) mFooterView
						.findViewById(R.id.message_foot_text);
				switch (getState()) {
				case STATE_LOAD_MORE:
				...
				case STATE_NETWORK_ERROR:
					...
					progress.setVisibility(View.GONE);
					text.setVisibility(View.VISIBLE);
					if (NetConnection.isNetWorkConnected(parent.getContext())) {
						text.setText("加载出错了");
					} else {
